...
- Scrapling, Synapse, and Sonar with uv
...
- Synapse/Sonar refs via `git ls-remote`
...
Config is mounted as a directory (`secrets/` -> `/config`), not a single file. OpenClaw can rewrite config atomically without hitting bind-mount inode locks.
...
`container/entrypoint.sh` runs before OpenClaw starts. It:
