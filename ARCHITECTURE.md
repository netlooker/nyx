...
**Sonar** now lives in the same thin runtime layer as OpenClaw and Qwen Code, not in the Nix base. Nyx installs it at image-build time from the public Sonar repo into an isolated app-layer virtualenv and bakes a default `sonar.toml` into the image. The current runtime install is locked to a verified Sonar git commit, and Nyx records both the requested ref and the resolved commit in build metadata. The Sonar binaries live under `/opt/sonar/bin/` (`/opt/sonar/bin/sonar-mcp`, `/opt/sonar/bin/sonar-api`, `/opt/sonar/bin/sonar-smoke`) and are also exported onto PATH. As with Synapse, Nyx now prefers those absolute paths in config so agent subprocesses do not depend on shell PATH inheritance. `/config/sonar.toml` follows the same host-override pattern as Synapse, while the decision to pin Sonar in `flake.nix` stays deferred until the runtime layer has been battle tested longer.
