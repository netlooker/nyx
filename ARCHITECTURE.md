...
**Synapse** lives in the pinned Nix base layer as a `buildPythonApplication` derivation inside `flake.nix`. It is fetched from GitHub by rev + sha256, built into the Nix store alongside git/jq/ripgrep, and included in `basePaths`. The Synapse binaries are available under `/nix-env/bin/` (`/nix-env/bin/synapse-mcp`, `/nix-env/bin/synapse-index`, `/nix-env/bin/synapse-search`) and also happen to be on PATH. Nyx treats the absolute `/nix-env/bin/...` locations as the stable contract for config and verification so child processes are not sensitive to stripped PATHs. Bumping Synapse is a one-shot flow: `just update-synapse` fetches the latest `main` commit, prefetches its tarball hash via `nix-prefetch-github`, and rewrites `flake.nix` in place. `just build` then reads the resolved rev via `nix eval .#packages.<system>.synapse.src.rev` and forwards it as image metadata — the value is documentation only, the actual pin lives in the flake.

**SearXNG** sits on the other side of that boundary for now. It is not pinned in `flake.nix` and it is not installed into the Nyx image. Instead, Nyx runs it as a private compose sidecar with a tracked `container/searxng/settings.yml` and an internal service URL (`http://searxng:8080`).

**Sonar** now lives in that same pinned Nix base layer. Like Synapse, it is a `buildPythonApplication` derivation inside `flake.nix`, fetched from GitHub by rev + sha256 and included in `basePaths`. The Sonar binaries live under `/nix-env/bin/` (`/nix-env/bin/sonar-mcp`, `/nix-env/bin/sonar-api`, `/nix-env/bin/sonar-smoke`) and Nyx prefers those absolute paths in config so agent subprocesses do not depend on shell PATH inheritance. Nyx also exposes a small `/nix-env/bin/sonar-python` wrapper for internal helper scripts that import Sonar modules directly. `/config/sonar.toml` follows the same host-override pattern as Synapse. Bumping Sonar is now the same style of flow as Synapse: `just update-sonar` fetches the latest `main` commit, prefetches the tarball hash, and rewrites `flake.nix`.
