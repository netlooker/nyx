...
**Sonar** now lives in the same thin runtime layer as OpenClaw and Qwen Code, not in the Nix base. Nyx installs it at image-build time from the public Sonar repo into an isolated app-layer virtualenv and bakes a default `sonar.toml` into the image. The current runtime install is locked to a verified Sonar git commit, and Nyx records both the requested ref and the resolved commit in build metadata. That lets agents use `sonar-mcp` immediately, with `/config/sonar.toml` following the same host-override pattern as Synapse, while keeping the decision to pin Sonar in `flake.nix` for a later phase after runtime battle testing.
