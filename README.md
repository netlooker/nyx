...
5. **Run:** Deploy the container anywhere! Map the internal `/data` volume to your host to ensure the agent's memory persists forever, and map `openclaw.json5` so you can hot-reload your configuration without rebuilding! (`docker run -d -v ./nyx-data:/data -v $(pwd)/nyxclaw_env/secrets/openclaw.json5:/app/nyxclaw_env/secrets/openclaw.json5 nyxclaw-agent`).
